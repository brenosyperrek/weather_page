...
export default function CitySelector({ capitais, cidadeSelecionada, aoSelecionar }: Props) {
  return (
    <label className="glass-card flex items-center gap-3 px-5 py-3 text-sm">
      <span className="text-white/50">Cidade</span>
      <select
        value={cidadeSelecionada}
        onChange={(evento) => aoSelecionar(evento.target.value)}
        className="cursor-pointer bg-transparent font-medium text-white outline-none [&>option]:bg-surface-800"
      >
